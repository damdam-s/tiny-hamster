...
class TinyXMLRPC:
    ...
    def _connect(self):
        ...
        self._common_server = ServerProxy(
            os.path.join(
                self.base_url,
                "common"),
            context=ssl_context)
        self._object_server = ServerProxy(
            os.path.join(
                self.base_url,
                "object"),
            context=ssl_context)
        self._wizard_server = ServerProxy(
            os.path.join(
                self.base_url,
                "wizard"),
            context=ssl_context)
    ...
    def execute(self, *params):
        return self._object_server.execute(
            self.database, self.user_id, self.password, *params)
...
class TinyServer (TinyXMLRPC):
    ...
    def search_project(self, search, timesheetable_only):
        if timesheetable_only:
            ...
            conds = []
        return self.execute(
            "project.project", "name_search", search, conds, "ilike", {
                "lang": "fr_FR", "name_search": search})

    def search_task(self, account_id, search):
        return self.execute("project.task", "name_search", search,
                            [["state", "=", "open"]], "ilike",
                            {"lang": "fr_FR", "name_search": search,
        ...
        return self.execute(
            "hr_timesheet_sheet.sheet", "search", [
                ["user_id", "=", self.user_id], [
                 ...
                 "date_from", "<=", date]],
            0.0, 80.0, 0,
            {"lang": "fr_FR", "active_ids": [], "active_id": 480}
        )

    def timesheet_defaults(self):
        return self.execute("hr.analytic.timesheet",
                            "default_get",
                            ["user_id",
                             "account_id",
                             "general_account_id",
    ...
    def attendance_defaults(self):
        return self.execute(
            "hr.attendance", "default_get", [
                "action", "employee_id"], {
                "lang": "fr_FR", "user_id": self.user_id})

    def current_timesheet_wiz(self):
        return self.wizard(208,
                           {"form": {},
                            "ids": [],
                               "report_type": "pdf",
                               "model": "ir.ui.menu",
    ...
    def invoice_factor(self, something):
        return self.execute("hr_timesheet_invoice.factor",
                            "name_get", [something], {"lang": "fr_FR"})
...
#        return self.execute("hr_timesheet_sheet.sheet",
#                            "create",
...
#                             "active_id": active_id})
    ...
    def timesheet_write(self, ts_id, ts_lines, att_lines=[]):
        return self.execute(
            "hr_timesheet_sheet.sheet", "write", [ts_id],
            {"timesheet_ids": ts_lines, "attendances_ids": att_lines},
            {"lang": "fr_FR", "active_ids": [],
             "read_delta": 0.0, "active_id": 0}
        )
