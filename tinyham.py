...
cur.execute("""
SELECT start_time, end_time, activities.name,
    description, categories.name, tags.name
FROM facts
...
for key, infos in activities.items():
    ...
    if len(match_proj) == 0:
        print ("No account project for '%s'"
               "in OpenERP. Please fix." % (category))
        sys.exit(1)
        ...
        if category in names:
            ...
            print ("Multiple projects found for '%s'"
                   "in OpenERP. Please fix." % (category))
            sys.exit(1)
    else:
    ...
    if tag is None:
        ...
        if len(match_task) == 0:
            print ("No task found for '%s' in account '%s' in OpenERP. "
                   "Please fix." % (tag, category))
            sys.exit(1)
        elif len(match_task) > 1:
            print ("Multiple tasks found for '%s' in account '%s' in OpenERP. "
                   "Please fix." % (tag, category))
            sys.exit(1)
        else:
    ...
    amount = tiny.on_change_unit_amount(
        ts_def["product_id"],
        duration_hours,
        ts_def["product_uom_id"])["value"]["amount"]
